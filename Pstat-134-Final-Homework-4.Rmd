...
Here we have  two random variables X1 ∼ Exp(1) and X2 ∼ Exp(3). The values 2 and 3 are rate parameters. The correlation coeffcient of them is ρ(X1, X2) = 0.7.

(a) 

The question asks us to generate 100 pairs of samples under each case based on our algorithm. First we will be defining the mean of our dimension vector. Then we need to create the covaraince matrix. In this case the correlation coeffcient of ρ(X1, X2) = 0.7. We compute the diagonal since our covariance matrix has 0.7 in off diagonal terms since we want Cov(Xi,Xj)=0.7 for all i,j.
...
We are now computing the CDF of each sample. We are working with the assumtion that each compnenet is normal mean 0 and variance 1 due to the mean vector and diagonal of the covariance matrix. We are computing the CDF of each sample by using the pnorm function which gives the distribtuon function of our gauss_vec which we created above.

```{r, cache=TRUE, warning=FALSE, message=FALSE}
unif_samples<- pnorm(gauss_vec)
```

Lastly I am going to be  using qexp in oder to obtain correlated 100 samples of Exp(2) distribution and Exp(3) distribution. We are using the same code for both X1 and X2. We are using the qexp function to return the corresponding values of the quantile function. Within this function we are inputting our CDF of each sample with the rate of paramaters which in this case it is X1~ Exp(2) and X2~Exp(3)
...
Furthermore let's look at histograms of the X1 and X2 and see them side by side. We will be using the qplot function in r and graphing two histograms side by side. From this we can see that the two have very similar histograms. They both follow very similar trends and have a similar distrbution shape. 
...
Now we are defining the variables without our target 
```{r, cache=TRUE, warning=FALSE, message=FALSE}
x<- MHsim(target_dist,10000,3,1)
y<- MHsim(target_dist,10000,3,10)
z<- MHsim(target_dist,10000,3,100)
...
maxz=max(c(x,y,z))
hist(x,breaks=seq(0,maxz,length=20))
hist(y,breaks=seq(0,maxz,length=20))
hist(z,breaks=seq(0,maxz,length=20))
```
